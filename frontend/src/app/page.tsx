...
export default function ChatPage() {
  ...
  const submit = useCallback(
    async (problem: string) => {
      ...
      try {
        for await (const event of streamSolve(text, { tier, signal: controller.signal })) {
          if (event.type === "stage") {
    ...
    <main className="mx-auto flex min-h-dvh max-w-3xl flex-col px-4 sm:px-6">
      <header className="pt-10 pb-6">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-muted">
          Every answer is recomputed by a computer algebra system before you see
          it. If the check fails, you are told.
        </p>
      </header>
